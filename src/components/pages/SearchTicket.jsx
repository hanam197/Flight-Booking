  ...
  const onSubmit = (formValues) => {
    searchBooking
      ...
      .then((res) => {
        ...
        if (!data) {
          // Dữ liệu không hợp lệ hoặc không có thông tin đặt chỗ
          setBooking({ error: "Không tìm thấy thông tin đặt chỗ!" });
          return;
        }
        // Cập nhật thời gian khởi hành và hạ cánh cho tất cả các vé
        data.tickets.forEach((ticket) => {
          const takeOffTime = new Date(ticket.flightId.takeOffTime);
          const landingTime = new Date(ticket.flightId.landingTime);
          ticket.flightId.takeOffTime = takeOffTime.toLocaleString();
          ticket.flightId.landingTime = landingTime.toLocaleString();
        });
        setBooking(data);
      })
      .catch((error) => {
        setBooking({ error: "Đã xảy ra lỗi khi tìm kiếm thông tin đặt chỗ!" });
      });
  };
      ...
      <div className="searchTicketContainer">
        <div className="searchTicketCard">
          <div className="formLeft">
            ...
            <p>
              Bạn muốn xem chuyến bay đã đặt vui lòng điền thông tin bên dưới:
            </p>
            <form
